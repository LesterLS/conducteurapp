...
export default function HalloConducteur() {
  ...
  const plaatsMelding = async () => {
    await supabase.from('meldingen').insert({ treinstelnummer: treinstel });
    fetchMeldingen();
  };
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '2rem auto' }}>
      ...
      />
      {treinstel && (
        <Button primary onClick={plaatsMelding} style={{ margin: '1rem 0' }}>
          ...
          .map((m) => (
            <Card key={m.id} fluid>
              <Card.Content>
                ...
                <Card.Meta>om {new Date(m.created_at).toLocaleTimeString()}</Card.Meta>
                <Button compact size="tiny" floated="right" onClick={() => afvinken(m.id)}>
                  Afvinken
                </Button>
              </Card.Content>
            </Card>
